...
public sealed partial class NetNode
{
    ...
    private bool HandleLine(string line, int? senderId, out string? forwardLine)
    {
        ...
        if (line.StartsWith("BOSSTESTLEVEL|", StringComparison.OrdinalIgnoreCase))
        {
            var payload = line["BOSSTESTLEVEL|".Length..];
            if (TryParseBossTestLevelTeleportPayload(payload, senderId, forceSenderId, out var ev))
            {
                lock (_sync)
                {
                    _pendingBossTestLevelTeleports.Add(ev);
                    _hasRemote = true;
                }

                if (_role == NetRole.Host && senderId.HasValue)
                {
                    var safeLevel = ev.LevelId.Replace("|", "/").Replace("\r", string.Empty).Replace("\n", string.Empty).Trim();
                    forwardLine = $"BOSSTESTLEVEL|{ev.UserId.ToString(CultureInfo.InvariantCulture)}|{safeLevel}\n";
                }
            }
            return true;
        }
        ...
        if (line.StartsWith("BOSSDEBUGSTAT|", StringComparison.OrdinalIgnoreCase))
        {
            var payload = line["BOSSDEBUGSTAT|".Length..];
            if (TryParseBossDebugStatAdjustPayload(payload, senderId, forceSenderId, out var ev))
            {
                lock (_sync)
                {
                    _pendingBossDebugStatAdjusts.Add(ev);
                    _hasRemote = true;
                }
                ...
                {
                    forwardLine =
                        $"BOSSDEBUGSTAT|{ev.UserId.ToString(CultureInfo.InvariantCulture)}|{ev.TargetUserId.ToString(CultureInfo.InvariantCulture)}|{ev.LifeDelta.ToString(CultureInfo.InvariantCulture)}\n";
                }
            }
            return true;
        }
